...
class WebpayPlusMallController extends Controller
{
    ...
    public function createdMallTransaction(Request $request)
    {
        ...
        $resp = WebpayPlus\MallTransaction::build()->create($req["buy_order"], $req["session_id"],  $req["return_url"], $req["detail"]);
    ...
    public function commitmallTransaction(Request $request)
    {
        $req = $request->except('_token');
        $token = $req["token_ws"];
        $resp = WebpayPlus\MallTransaction::build()->commit($token);
    ...
    public function getMallTransactionStatus(Request $request)
    {
        $req = $request->except('_token');
        $token = $req["token"];
        $resp = WebpayPlus\MallTransaction::build()->status($token);
    ...
    public function refundMallTransaction(Request $request)
    {
        ...
        $token = $req["token"];
        try {
            $resp = WebpayPlus\MallTransaction::build()->refund($token, $req["buy_order"],$req["commerce_code"], $req["amount"]);
