...
class WebpayPlusMallDeferredController extends Controller
{
    ...
    public function createMallDiferido(Request $request)
    {
        ...
        $resp = MallTransaction::build()->create($req["buy_order"], $req["session_id"],  $req["return_url"], $req["detail"]);
    ...
    {
        $req = $request->except('_token');
        $resp = MallTransaction::build()->commit($req["token_ws"]);
    ...
    public function captureMallDiferido(Request $request)
    {
        ...
        $resp = MallTransaction::build()->capture($childCommerceCode, $token, $buyOrder, $authCode, $amount);
    ...
    public function refundMallDiferido(Request $request)
    {
        ...
        $resp = MallTransaction::build()->refund($token, $childBuyOrder, $childCommerceCode, $amount);
        dd($resp);
    }
    ...
    public function statusMallDiferido(Request $request)
    {
        $req = $request->except('_token');
        $token = $req["token"];
        $resp = MallTransaction::build()->status($token);
        dd($resp);
    }
}
